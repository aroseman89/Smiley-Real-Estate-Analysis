...
However, feel free to scroll through the R Markdown file on Github and take a 
look at the notes I left about why I made the corrections I did. But for now,
...
rate_bar <- ggplot(agent_market_summary_long, aes(x = Rate, y = market_id, fill = Close_Rate_Type,
                                   text = paste("Market:", market_id, "<br>",
                                                Close_Rate_Type, ":", scales::percent(Rate, accuracy = 0.1)))) +
  ...
  labs(
    title = "Average Buyer and Seller Close Rates by Market",
    subtitle = "Markets sorted by overall average closing performance",
...
than closing rates alone. Lots of factors can affect a market's performance - 
let's keep digging!
...
```
### 2.1.2 Most Efficient Markets 🚀

California and New York still may be overrepresented here. The previous scoring 
was still weighted heavily to the wealthiest and most densely populated states, 
despite normalization. But where is Smiley Real Estate most *efficient* rather 
than merely depending on population and wealth factors? The following analysis 
weights each factor according to importance: closing rates and customer
saturation are weighted more highly, while median closing price is weighted
lower to favor efficiency over property value or population.
...
After accounting for population and high property values, the top 5 markets are
Grand Rapids, Raleigh, Buffalo, San Jose, and Louisville. These markets have the
best balance between liquidity (they don't spend a lot of time on the market),
listing price (with the exception of San Jose, they present a low barrier to 
entry for a buyer), listing price increase rates, and agent closing rates and 
thus represent the best opportunity to maximize return on investment (ROI).

### 2.1.3 Market Data Trend 📈📉
...
In contrast, price volatility is highest in low turnover markets. These markets 
may be experiencing "boom" conditions due to a sudden lack of supply or
short-term enthusiasm, making them riskier. An investor is most likely to find 
consistent, steady returns - rather than speculative gains - in the 
high-turnover, low-volatility market.

```{r market_trend_chart, message=FALSE}
market_ranking_data %>%
  ...
  labs(
    ...
    x = expression("Turnover Rate"),
    y = expression("Price Increase Rate")
  ) +
  ...
  scale_color_gradient(
    low = "#09223d", 
    high = "#44b2ef", 
    name = expression("Days on Market")
  ) + 
...
top_10_agents_table %>%
  ...
  row_spec(0, bold = T, background = "#f2f2f2")
```
The table above highlights the top 10 agents overall. For a complete listing of 
the best agent in every market analyzed, the complete, ranked list can be 
downloaded here:
  <a href="https://your-company.com/data/best_agents_per_market.csv" target="_blank">Download Full Agent Ranking (CSV)</a>
...
customer_review_average_compare <- plot_ly(
  ...
  name = "Average Number of Customer Reviews",
  type = "bar",
  text = sprintf("%.1f reviews", c(mean_customer_reviews_2024, mean_top_agent_customer_reviews_2024)), 
  textposition = 'auto',
  hovertemplate = "%{x} have %{y:.1f} customer reviews<extra></extra>",
         marker = list(color = colors,line = list(color = 'rgb(8,48,107)', 
                                                  width = 1.5))
)
customer_review_average_compare <- customer_review_average_compare %>% layout(title = "Average Number of Customer Reviews Compared to Top Agents",
         xaxis = list(title = ""),
         yaxis = list(title = "")
)
...
layout(title = 'Top Agents with Higher than Average Number of Reviews',
...
total_raw_closes_compare <- plot_ly(
  ...
  type = "bar",
  text = sprintf("%.1f closed deals", c(mean_total_raw_closes, mean_top_agent_total_raw_closes)), 
  textposition = 'auto',
...
total_raw_closes_compare <- total_raw_closes_compare %>% layout(title = 
                                                                  "Average Number of Closed Deals Compared to Top Agents",
         xaxis = list(title = ""),
         yaxis = list(title = "")
)
...
layout(title = 'Top Agents with Higher than Average Number of Closed Deals',
         xaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE),
         yaxis = list(showgrid = FALSE, zeroline = FALSE, showticklabels = FALSE))
...
total_close_rate_compare <- plot_ly(data,
    ...
    type = "bar",
    hovertemplate = "%{x}<br>Buyer Rate: %{y:.1%}<extra></extra>",
    marker = list(
        color = colors[1],
...
total_close_rate_compare <- total_close_rate_compare %>%
    add_trace(
        ...
        hovertemplate = "%{x}<br>Seller Rate: %{y:.1%}<extra></extra>",
        marker = list(
            color = colors[2],
            line = list(color = 'rgb(8,48,107)', width = 1.5)
        )
...
total_close_rate_compare <- total_close_rate_compare %>%
    layout(
        title = "Average Close Rate Compared to Top Agents",
        xaxis = list(title = ""),
        yaxis = list(
            title = 'Close Rate',
            tickformat = '.0%'),
...
percent_seller_close_rate_chart <- plot_ly(
    ...
    name = "Seller Rate"
) %>% layout(title = 'Seller Close Rate')
...
combined_pie_charts <- combined_pie_charts %>%
    layout(
        title = 'Percentage of Top Agents Exceeding Average Close Rates',
        showlegend = FALSE
    )
...
data_case_study <- tibble(
  Market = c("New York, NY", "Grand Rapids, MI"),
...
raw_market_comparison <- ggplot(data_long_case_study, aes(x = Market, y = Value, fill = Market, text = paste0(Market, "
", Metric_Type, ": ", format(Value, big.mark = ",", scientific = FALSE)))) +
  geom_col(position = "dodge") +
  facet_wrap(~ Metric_Type, scales = "free_y", ncol = 3) +
  labs(
    title = "Raw Market Performance Comparison (NY vs GR)",
    y = "Value",
    x = ""
  ) +
  scale_fill_manual(values = c("New York, NY" = "#2a4a6a", "Grand Rapids, MI" = "#229fe6")) +
  theme_minimal() +
  theme(legend.position = "none",
        axis.text.x = element_text(angle = 45, hjust = 1))

chart1 <- ggplotly(raw_market_comparison, tooltip = "text") %>%
  layout(title = list(text = "Raw Market Performance Comparison (NY vs GR)"))
...
## 3.1 What If There Was More Time? ⌚
...
## 3.2 How did this report come together?
