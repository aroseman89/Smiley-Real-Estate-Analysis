...
#Note: Update all file paths for your directory.
...
This first section is about cleaning and preparing the data. It's not very interesting and isn't very nice to look at, so it's hidden from this report. However, feel free to [view the complete code](https://github.com/aroseman89/Smiley-Real-Estate-Analysis/blob/main/Clever%20Real%20Estate%20Working%20Interview%20Project.Rmd) to see the full cleaning steps and take a look at the notes I left about why I made the corrections I did. But for now, let's get straight to the fun part!
...
```{r spell_check, include=FALSE}
#Check unique id values to look for typos
agent_id_check <- unique(agents$agent_id)
#1000 elements and 1000 rows of raw data - all ids are unique
agent_market_id_check <- unique(agents$market_id)
#51 elements
call_id_check <- unique(calls$call_id)
#175359 elements and 175359 rows of raw data - all ids are unique
customer_id_check <- unique(customers$customer_id)
#50000 elements and 50000 rows of raw data - all ids are unique
customer_market_id_check <- unique(customers$market_id)
#50 elements <- there is an conflict with the number of market_ids
#In looking at unique entries, agents$market_id contains the erroneous "orlando_fla"
#Correct entry to "orlando_fl"
agents$market_id <- str_replace(string = agents$market_id, pattern = "_fla", replacement = "_fl")
#Rechecking agents$market_id - now contains 50 elements and matches customers$market_id
market_market_id_check <- unique(markets$market_id)
#50 elements - matches expectations established above
market_census_name_check <- unique(markets$census_name)
#50 elements - matches expectations
market_common_name_check <- unique(markets$common_name)
#50 elements - matches expectations
sale_id_check <- unique(sales$sale_id)
#14172 elements and 14172 rows of raw data - all ids are unique
```
...
#of total listings. Outliers appear to be accurate, extreme cases. Data should be
...
```{r closing_rate_average_chart, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
```{r unweighted_table, eval=if (knitr::is_html_output()) TRUE else FALSE}
#Prepare the data
...
```{r table_weighted, eval=if (knitr::is_html_output()) TRUE else FALSE}
top_5_score_table_data <- top_5_markets %>%
  
  #Ensure data is sorted by rank derived from the performance_score
  arrange(rank) %>%
  
  #Select new calculated metrics first, followed by raw metrics
  select(
    Rank = rank,                             
...
#Top 5 Best Markets For Efficiency
top_5_markets_weighted <- market_ranking_final_weighted %>%
  head(5)
```

```{r table_efficiency, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
```{r market_trend_chart, echo=FALSE, message=FALSE, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
```{r agent_table, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
<small>The table above highlights the top 10 agents overall. For a complete listing of the best agent in every market analyzed, the complete, ranked list can be viewed [here](https://github.com/aroseman89/Smiley-Real-Estate-Analysis/blob/main/best_agents_per_market.csv).</small>
...
```{r agent_success_customer_engagement_chart, echo=FALSE, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
```{r agent_success_volume_chart, eval=if (knitr::is_html_output()) TRUE else FALSE}
colors <- c('#44b2ef', '#09223d')
...
```{r agent_success_close_chart, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
```{r case_study, eval=if (knitr::is_html_output()) TRUE else FALSE}
...
Metric == "Turnover_Rate" ~ paste0(format(round(Value * 100, 2), big.mark = ",", scientific = FALSE, trim = TRUE, nsmall = 2), "%"),
 ...
 size = 3,
fontface = "bold",
    color = "white" # Retained white color from previous step
 ) +
